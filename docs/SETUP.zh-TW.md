...
| 項目 | 路徑 | 說明 |
|---|---|---|
| copilot-api | `~/.bun/bin/copilot-api` 或 npm global bin | 本機 gateway |
...
`claude-copilot models` 會同時抓兩個 endpoint，寫成合併 catalog，並印出各模型回報的 context window、prompt/output 上限、context tiers、reasoning effort：
